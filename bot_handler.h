...
private:
    ...
    void sendDepartmentMenu(int64_t chatId);                     // <-- добавить
    void sendCourseMenu(int64_t chatId, int department);        // <-- добавить (2 аргумента)
    void sendGroupMenu(int64_t chatId, int course);
    void sendSchedule(int64_t chatId, int course, int group);
    void sendExternalLink(int64_t chatId, int course, int department); // <-- добавить
